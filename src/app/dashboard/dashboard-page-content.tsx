...
import { ArrowRight, BarChart2, Clock, Database, Trash, Trash2, TrashIcon } from "lucide-react"
...
export const DashboardPageContent = () => {
    const [deleteingCategory, setDeletingCategory] = useState<string | null>(null)
                ...
                <div className="pointer-events-none z-0 absolute inset-px rounded-lg shadow-sm transition-all duration-300 group-hover:shadow-md ring-1 ring-black/5" />
                    <div className="relative p-6 z-10">
                        ...
                        <div className="flex items-center justify-between mt-4">
                            <Link href={`/dashboard/category/${category.name}`} className={buttonVariants({variant: "outline", size: "sm", className: "flex items-center gap-2 text-sm"})}>View all <ArrowRight className="size-4"/></Link>
    ...
    <Modal showModal={!!deleteingCategory} setShowModal={() => setDeletingCategory(null)} className="max-w-md p-8">
        <div className="space-y-6">
            <div>
                <h2 className="text-lg/7 font-medium tracking-tight text-gray-950">Delete category</h2>
                <p className="text-sm/6 text-gray-600">
                    Are you sure you want to delete the category "{deleteingCategory}" ? This action cannot can be undone.
                </p>
            </div>
            ...
            <div className="flex justify-end space-x-3 pt-4 border-t">
                <Button variant="outline" onClick={() => setDeletingCategory(null)}>
                    ...
                    <Button variant="destructive" onClick={() => deleteingCategory && deleteCategory(deleteingCategory)} disabled={isDeletingCategory}>{isDeletingCategory ? "Deleting..." : "Delete"}</Button>
            </div>
        </div>
    </Modal>
    </>
